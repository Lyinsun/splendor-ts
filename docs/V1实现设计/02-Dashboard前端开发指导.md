...
- 标准场地元素必须有稳定尺寸：选择精灵球、弃球、计数、按钮禁用态、卡牌文案或当前玩家变化时，只更新内容和状态，不改变资源区、市场行、牌堆槽、卡槽和特殊区槽位的占位。
- 拿球失败必须在资源银行附近展示可理解原因，例如组合不合法、两枚同色银行不足 4、银行数量不足、超过 10 枚需要先选择弃球、弃球选择不匹配或当前不是控制席位回合；服务端错误码仍是最终依据。
