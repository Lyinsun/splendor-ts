...
- `creature-academy` 原创收集灵兽主题，包含主题 hero 与按元素/等级裁切的卡牌插画。
...
- 卡图采用 `cards/<element>-t<tier>.png` 的展示路径约定，当前由 `creature-academy` 主题提供。该路径只由前端 `presentation` 层根据服务端卡牌 id、元素和等级派生，不能反向驱动规则。
